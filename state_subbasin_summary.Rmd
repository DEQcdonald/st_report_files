...
There are `r length(unique(par_sum_au$AU_ID))` assessment units within the `r subbasin_name` subbasin that had stations with available data to be included in this report. Some assessment units did not contain stations with targets or sufficient available data to determine attainment status and are listed as unassessed. `r tbls(name = paste0(subbasin_name, "_au_sum"), display="cite")` and `r figs(name = paste0(subbasin_name, "_au_sum_plot"), display="cite")` summarizes the number of attaining, not attaining, and unassessed assessment units by parameter for the current status period.
...
Oregon DEQ groups assessment units into five types: "Stream", "Watershed Unit", "Lake", "Estuary or Bay", and "Coastline". `r tbls(name = paste0(subbasin_name, "_au_type_sum"), display="cite")` summarizes of the number of attaining, not attaining, and unassessed assessment units by parameter and assessment unit type.
...
There are `r length(unique(par_sum$MLocID))` stations within the `r subbasin_name` subbasin with available data included in this report. Stations without targets or without sufficient data to determine attainment are listed as unassessed. The reason for a station's unassessed status can be found in the appendix for this area. `r tbls(name = paste0(subbasin_name, "_stn_sum"), display="cite")` and `r figs(name = paste0(subbasin_name, "_stn_sum_plot"), display="cite")` summarizes the number of attaining, not attaining, and unassessed stations by parameter for the current status period.
