...
if(name == "Columbia River"){
  ...
  basin_wql_streams_lines_i <- basin_wql_streams_lines_i %>% dplyr::filter(AU_ID %in% columbia_aus)
  basin_wql_streams_ws_i <- basin_wql_streams_ws_i %>% dplyr::filter(AU_ID %in% columbia_aus)
  ...
  basin_wql_streams_lines_i <- basin_wql_streams_lines_i %>% dplyr::filter(AU_ID %in% snake_aus)
  basin_wql_streams_ws_i <- basin_wql_streams_ws_i %>% dplyr::filter(AU_ID %in% snake_aus)
...
cat(paste0("The ", name, " is listed for ", length(unique(wql_data$Char_Name)), " parameters among its water bodies. Available data were sufficient to assess status and/or trend at ", length(unique(param_sum$MLocID)), " stations within the ", name, ". These stations were located across ", length(unique(param_sum_au$AU_ID)), " assessment units consisting of '", list_fun(types[1:(n_types - 1)]), "s' and '", list_fun(types[n_types]), "s'. Data for ",
           gsub("'", "", 
...
knitr::kable(au_sum, padding = 0, digits = 1, row.names = FALSE, format = table_format,
             caption = tbls(name = paste0(name, "_au_sum"), caption = paste0("Summary of assessment unit status within the ", name, "."))
) %>% table_style()
...
cap_list[[1]] <- figs(name = paste0(name, "_au_sum_plot"), caption = paste0("Summary plot of assessment unit status within the ", name, " by parameter."))

cat(paste0("\n\n",tbls(name = paste0(name, "_au_type_sum"), display = 'cite')," and ", figs(name = paste0(name, "_au_sum_plot"), display = 'cite')," summarizes the number of attaining, not attaining, and unassessed assessment units by parameter and assessment unit type within the ", name,".\n\n"))
...
knitr::kable(au_sum_type, padding = 0, digits = 1, row.names = FALSE, format = table_format
             ,
             caption = tbls(name = paste0(name, "_au_type_sum"), caption = paste0(name, " status summary of assessment units by type."))
) %>% table_style()
...
knitr::kable(stn_sum, padding = 0, digits = 1, row.names = FALSE, format = table_format
             ,
             caption = tbls(name = paste0(name, "_stn_sum"), caption = paste0("Summary of the status of stations within the ", name, "."))
) %>% table_style()
...
cap_list[[2]] <- figs(name = paste0(name, "_stn_sum_plot"), caption = paste0("Summary plot of station status within the ", name, " by parameter."))
