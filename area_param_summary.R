...
area <- readOGR(dsn = "//deqhq1/GISLIBRARY/Base_Data/Hydrography/Watershed_Boundaries/WBD_OR.gdb/WBD_OR.gdb", 
...
au_names <- read.csv('//deqhq1/WQNPS/Status_and_Trend_Reports/Lookups_Statewide/AssessmentUnits_OR_Dissolve.txt', stringsAsFactors = FALSE)
  ...
  stations_AWQMS <- get_stations_AWQMS(area)
  ...
  if(any(unique(data_clean$Char_Name) %in% odeqstatusandtrends::AWQMS_Char_Names('pH'))){
    ...
    data_pH$Spawn_type <- "Not_Spawn" # Remove once column is added in to add_criteria()
  ...
  if(any(unique(data_clean$Char_Name) %in% odeqstatusandtrends::AWQMS_Char_Names('bacteria'))){
    ...
    data_bact$Spawn_type <- "Not_Spawn" # Remove once column is added in to add_criteria()
